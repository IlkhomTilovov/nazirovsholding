...
export default function Catalog() {
  ...
  const filters: ProductFilters = useMemo(() => {
    ...
    if (sidebarFilters.brandIds.length > 0) f.brandIds = sidebarFilters.brandIds;
  ...
  useEffect(() => {
    // Don't write URL until categories are loaded and any URL slug is resolved.
    // Otherwise we strip the incoming ?category=slug before it can be applied.
    if (categories.length === 0) return;
    if (resolvedCategoryId === null) return;
    // Wait until the sidebar state catches up with the resolved category from the URL.
  ...
  }, [sidebarFilters, currentPage, setSearchParams, filterOptions.maxPrice, categories, resolvedCategoryId]);
              ...
              <div className="text-center py-16">
                ...
                <Button
                  ...
                  onClick={() => handleApplyFilters({
                    categoryId: 'all', brandIds: [], priceMin: 0, priceMax: filterOptions.maxPrice,
