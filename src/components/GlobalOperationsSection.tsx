...
export function GlobalOperationsSection() {
  ...
  return (
    <section ref={sectionRef} className="relative bg-background text-foreground overflow-hidden">
        ...
        <div className="relative h-full max-w-[1280px] mx-auto px-6 lg:px-10 flex flex-col justify-end pb-16 md:pb-24">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <EditableText contentKey="bozorlar.hero.eyebrow" fallback="Global Operations" section="bozorlar-hero" className="text-primary text-[11px] tracking-[0.45em] uppercase font-medium" />
            </div>
            <EditableText as="h2" contentKey="bozorlar.hero.title" fallback="Biz natijalar orqali gapiramiz" section="bozorlar-hero" className="font-serif text-4xl md:text-6xl lg:text-7xl leading-[1.05] text-foreground" />
            <EditableText as="p" multiline contentKey="bozorlar.hero.desc" fallback="NazirovSholding xalqaro savdo, eksport, logistika va strategik hamkorliklar orqali global bozorlarda faoliyat yuritadi. Bizning yondashuvimiz ishonch, natija va uzoq muddatli hamkorlikka asoslangan." section="bozorlar-hero" className="mt-7 text-base md:text-lg text-foreground/70 max-w-2xl leading-relaxed" />
          </motion.div>
        </div>
      </div>
        ...
        <div className="relative max-w-[1280px] mx-auto px-6 lg:px-10 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-end">
          ...
          >
            <div className="flex items-center gap-3 mb-8">
              <span className="text-primary font-serif italic text-lg">—  01</span>
              <EditableText contentKey="bozorlar.msg.eyebrow" fallback="Corporate Message" section="bozorlar-msg" className="text-foreground/40 text-[11px] tracking-[0.4em] uppercase" />
            </div>
            <EditableText as="h3" multiline contentKey="bozorlar.msg.title" fallback="Connecting Uzbekistan To Global Markets" section="bozorlar-msg" className="font-serif text-4xl md:text-6xl lg:text-[5.5rem] leading-[1.02] text-foreground" />
          </motion.div>
          <motion.div
            variants={fadeUp} custom={1} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }}
            className="lg:col-span-5 lg:pb-6"
          >
            <EditableText as="p" multiline contentKey="bozorlar.msg.desc" fallback="NazirovSholding creates strategic connections between manufacturers, suppliers, international buyers and emerging markets through export, logistics and investment partnerships." section="bozorlar-msg" className="text-foreground/70 text-lg leading-[1.8]" />
            <div className="mt-10 h-px w-24 bg-primary/60" />
          </motion.div>
        </div>
      </div>

      {/* ───────── BLOCK 3 — BUSINESS STATISTICS ───────── */}
      <div className="relative py-28 md:py-36 bg-background">
        <div className="max-w-[1280px] mx-auto px-6 lg:px-10">
          <motion.div
            variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.4 }}
            className="flex items-center gap-3 mb-16"
          >
            <span className="text-primary font-serif italic text-lg">—  02</span>
            <EditableText contentKey="bozorlar.stats.eyebrow" fallback="Performance Indicators" section="bozorlar-stats" className="text-foreground/40 text-[11px] tracking-[0.4em] uppercase" />
          </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-primary/10 border border-primary/10">
            {[
              ...
              <motion.div
                ...
                className="group relative bg-background p-10 md:p-12 transition-all duration-500 hover:bg-background"
              >
                <div className="absolute top-6 right-6 text-primary/30 text-xs tracking-[0.3em] font-mono">{s.k}</div>
                <div className="absolute left-0 top-0 h-full w-px bg-primary origin-top scale-y-0 group-hover:scale-y-100 transition-transform duration-700" />
                <EditableText
                  as="div"
      ...
      <div className="relative py-28 md:py-36 bg-background">
        <div className="max-w-[1320px] mx-auto px-6 lg:px-10">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              <span className="text-primary font-serif italic text-lg">—  04</span>
              <EditableText contentKey="bozorlar.cases.eyebrow" fallback="Case Studies" section="bozorlar-cases" className="text-foreground/40 text-[11px] tracking-[0.4em] uppercase" />
            </div>
            <EditableText as="h3" contentKey="bozorlar.cases.title" fallback="Featured Global Projects" section="bozorlar-cases" className="font-serif text-4xl md:text-5xl lg:text-6xl text-foreground leading-[1.05]" />
          </motion.div>
            ...
            {[
              ...
              >
                ...
                <div className="md:col-span-7 p-8 md:p-12 lg:p-14 flex flex-col justify-between gap-8">
                  <div>
                    <EditableText contentKey={`bozorlar.case.${p.id}.industry`} fallback={p.industry} section="bozorlar-cases" className="block text-primary text-[11px] tracking-[0.35em] uppercase mb-4" />
                    <EditableText as="h4" contentKey={`bozorlar.case.${p.id}.title`} fallback={p.title} section="bozorlar-cases" className="font-serif text-3xl md:text-4xl text-foreground leading-tight mb-6 group-hover:text-primary transition-colors duration-500" />
                    <div className="h-px w-16 bg-primary/40 group-hover:w-28 transition-all duration-700" />
                  </div>

                  <div className="grid grid-cols-2 gap-8 pt-4">
                    <div>
                      <EditableText contentKey="bozorlar.case.label.result" fallback="Result" section="bozorlar-cases" className="block text-[10px] tracking-[0.4em] uppercase text-foreground/40 mb-2" />
                      <EditableText contentKey={`bozorlar.case.${p.id}.result`} fallback={p.result} section="bozorlar-cases" className="block font-serif text-xl text-foreground" />
                    </div>
                    <div>
                      <EditableText contentKey="bozorlar.case.label.year" fallback="Year" section="bozorlar-cases" className="block text-[10px] tracking-[0.4em] uppercase text-foreground/40 mb-2" />
                      <EditableText contentKey={`bozorlar.case.${p.id}.year`} fallback={p.year} section="bozorlar-cases" className="block font-serif text-xl text-primary" />
                    </div>
                  </div>
                </div>

                <div className="absolute bottom-0 left-0 h-px bg-primary w-0 group-hover:w-full transition-all duration-1000 ease-out" />
              </motion.article>
            ))}
          </div>
        </div>
